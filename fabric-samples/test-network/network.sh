...
function checkPrereqs() {
  ...
  DOCKER_IMAGE_VERSION=$(${CONTAINER_CLI} run --rm hyperledger/fabric-tools:2.4.5 peer version | sed -ne 's/^ Version: //p')
...
# file for creating the network is "docker-docker-compose-test-net.yaml" in the ``docker``
...
function deployCCAAS() {
  echo "========================\n" $CHANNEL_NAME $CC_NAME $CC_SRC_PATH $CCAAS_DOCKER_RUN $CC_VERSION $CC_SEQUENCE $CC_INIT_FCN $CC_END_POLICY $CC_COLL_CONFIG $CLI_DELAY $MAX_RETRY $VERBOSE $CCAAS_DOCKER_RUN
